...
export function CommentSection({ task }: { task: TaskDetail }) {
  ...
  function switchMode(next: Mode) {
    setMode(next);
    // Best-effort convenience: most client tasks have their contact email typed
    // somewhere in the brief/description — there's no structured field for it.
    if (next === "EMAIL" && !to) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2.5 pt-2">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <RichTextComposer
            placeholder={mode === "EMAIL" ? "Write your email…" : "Leave a comment…"}
            submitLabel={mode === "EMAIL" ? "Send email" : "Comment"}
